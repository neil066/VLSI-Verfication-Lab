        ...
        *and* from body‑level declarations such as
        ...
        while i < len(lines):
            ...
            if ln.startswith("module"):
                ...
                if m:
                    ...
                    for item in port_list:
                        # Check for keywords to set the current direction
                        # This correctly handles mixed/complex declarations.
                        temp_item = item.strip()
                        if temp_item.startswith("input"):
                            direction = "input"
                        elif temp_item.startswith("output"):
                            direction = "output"
            ...
            if ln.startswith("endmodule"):
                if current:  # If we were just inside a module
                    # Improvement: Remove duplicates while preserving order
                    seen_inputs = set()
                    current['inputs'] = [x for x in current['inputs'] if not (x in seen_inputs or seen_inputs.add(x))]
                    seen_outputs = set()
                    current['outputs'] = [x for x in current['outputs'] if not (x in seen_outputs or seen_outputs.add(x))]
                current = None
                i += 1
                continue
